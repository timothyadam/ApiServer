...
class DbBase(object):
    ...
    def create_insert_sql(self, db_name, table_name, fields, tuple):
        ...
        sql = 'insert into %s.%s %s values %s' % (
            db_name, table_name, fields, tuple)
    ...
    def create_get_page_sql(
        ...
        if (condition is None) or (condition == "") or (condition == " "):
            ...
            sql += " where " + condition + \
                " limit %s,%s" % (start_page, page_size)
        ...
        if db_name:
            ...
            if relations:
                for relation in relations:
                    sql += " left join " + db_name + "." + \
                        relation["table_name"] + " on " + relation["join_condition"]
            if condition is None:
                sql = sql
            else:
                sql += " where " + condition
    ...
    def create_get_relation_page_sql(
            self,
            db_name,
            table_name,
            fields,
    ...
    def insert_sql(self, db_name, table_name, data_dict):
        ...
        if len(insert_data) == 1:
            ...
            insert_data = str(tuple(insert_data))
        sql = 'insert into %s.%s %s values %s' % (
            db_name, table_name, fields, insert_data)
